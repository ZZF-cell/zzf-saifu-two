...
| **隐私保护** | 年龄确认门禁、匿名包装、隐藏商品名、订单销毁、AES-256-GCM 加密、手机号哈希 |
...
> **隐私选项（M12）**：`anonymousPackaging`（匿名包装配送，面单不含商品/品牌信息）与 `hideProductName`（隐藏商品名）默认全开，结算页可逐项关闭，随 `createOrder` 落库到 `Order.privacy`。`hideProductName=true` 时，用户侧订单列表首件商品名与订单详情逐行商品名统一显示为「私密商品」——掩码实现在**查询层**（`orders.queries.ts` 一处），列表/详情/任何消费端一致生效；品牌方订单列表与管理后台不经过该掩码（商家履约需要真实商品名）。
